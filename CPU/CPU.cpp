...
CPUerror CPU::ret_code() { return context.exit_code; }
...
#define printLongRegInfo(regName) dump_log_reg(#regName, &context.Register.regName);
    printLongRegInfo(lr0);
    printLongRegInfo(lr1);
    printLongRegInfo(lr2);
    printLongRegInfo(lr3);
    printLongRegInfo(lr4);
    printLongRegInfo(lr5);
    printLongRegInfo(lr6);
    printLongRegInfo(lr7);
#undef printLongRegInfo

    fprintf(outStream, "    }\n");
#ifdef DUMP_PRINT_MEMORY
    fprintf(outStream, "    RAM:\n");
    fprintf(outStream, "    Segment offset |  0x00  0x01  0x02  0x03  0x04  0x05  0x06  0x07  0x08  0x09  0x0A  0x0B  "
                       "0x0C  0x0D  0x0E  0x0F\n");
    fprintf(outStream, "    "
                       "-----------------------------------------------------------------------------------------------"
                       "-----------------\n");
